"""
00_clean_gharchive_year.py
--------------------------
Generic cleaner for any GH Archive year.

Usage:
  spark-submit /tmp/00_clean_gharchive_year.py <year>

  e.g.:
  spark-submit /tmp/00_clean_gharchive_year.py 2022
  spark-submit /tmp/00_clean_gharchive_year.py 2023

Input:  /user/jl17797_nyu_edu/oss_pulse/source/gharchive_<year>/
Output: /user/jl17797_nyu_edu/oss_pulse/cleaned/gharchive_<year>/
        (partitioned by event_date, same schema as by2566's 2025 data)
"""
...
# argv[1] can be "2022" (full year) or "2022/month_03" (single month)
arg  = sys.argv[1] if len(sys.argv) > 1 else "2022"
year = arg.split("/")[0]

spark = SparkSession.builder.appName(f"00_CleanGHArchive{arg.replace('/', '_')}").getOrCreate()
...
CORE_EVENT_TYPES = ["WatchEvent", "ForkEvent", "PushEvent", "PullRequestEvent", "IssuesEvent"]

# ── Read ──────────────────────────────────────────────────────────────────────

IN  = f"/user/jl17797_nyu_edu/oss_pulse/source/gharchive_{arg}"
OUT = f"/user/jl17797_nyu_edu/oss_pulse/cleaned/gharchive_{year}"

print(f"[INFO] Reading {year} raw from {IN}")
raw = spark.read.schema(gharchive_schema).json(IN)
