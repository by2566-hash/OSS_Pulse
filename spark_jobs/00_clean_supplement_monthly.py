"""
00_clean_supplement_monthly.py
-------------------------------
Clean one month of supplement data to avoid YARN lifetime timeout.

Usage:
  spark-submit /tmp/00_clean_supplement_monthly.py <YYYY-MM> <overwrite|append>

  e.g.:
  spark-submit /tmp/00_clean_supplement_monthly.py 2025-12 overwrite
  spark-submit /tmp/00_clean_supplement_monthly.py 2026-01 append

Input:  /user/jl17797_nyu_edu/oss_pulse/source/gharchive_raw/<YYYY-MM>-*.json.gz
...
write_mode = sys.argv[2] if len(sys.argv) > 2 else "append"
...
# ── Read only this month's files ──────────────────────────────────────────────

IN  = f"/user/jl17797_nyu_edu/oss_pulse/source/gharchive_raw/{month}-*.json.gz"
OUT = "/user/jl17797_nyu_edu/oss_pulse/cleaned/gharchive_supplement"

print(f"[INFO] Reading {month} from {IN}")
...
cleaned.write.mode(write_mode).partitionBy("event_date").parquet(OUT)

print(f"[INFO] {month} done. Rows: {row_count}. Mode: {write_mode}. Output: {OUT}")
spark.stop()
